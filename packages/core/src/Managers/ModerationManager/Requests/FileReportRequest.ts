...
// escalates at once instead of waiting in the open queue. A visitor's report carries
// the same submission an anonymous comment does, so it passes the same guard (D15):
// Turnstile, the block list and the rate limit. A member's report needs none.
export class FileReportRequest extends RequestBase {
  constructor(
    ...
    readonly submission: AnonymousSubmission | undefined,
