...
export async function permit(
...
): Promise<NotificationForbiddenResponse | NotificationUnavailableResponse | undefined> {
  ...
  if (verdict instanceof PermissionDeniedResponse) {
    return new NotificationForbiddenResponse(correlationId, verdict.reason);
  }
  if (verdict instanceof PermissionUnavailableResponse) {
    return new NotificationUnavailableResponse(correlationId, verdict.reason);
  }
