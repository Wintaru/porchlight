...
// Every post handler asks the same question first. Answers `undefined` when the actor
...
export async function permit(
...
): Promise<PostForbiddenResponse | PostUnavailableResponse | undefined> {
  ...
  if (verdict instanceof PermissionDeniedResponse) {
    return new PostForbiddenResponse(correlationId, verdict.reason);
  }
  if (verdict instanceof PermissionUnavailableResponse) {
    return new PostUnavailableResponse(correlationId, verdict.reason);
  }
